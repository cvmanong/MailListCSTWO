...
public class BookService {
    ...
    // 实现添加逻辑处理
    public boolean addBook(String id, String name, double price, int number,String address){
        // 遍历数据，判断要插入的水果编号是否存在
        ...
        Book thisbook=new Book(id,name,price,number,address);
     ...
     * @return
     */
    public boolean updateBook(String id, String name, double price, int number,String address) {
        // 遍历数据，判断要插入的水果编号是否存在
        ArrayList<Book> data=queryAll();
        for(int i=0;i<data.size();i++){
            Book book=data.get(i);
